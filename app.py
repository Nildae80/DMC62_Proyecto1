...
if modulos == "Home":
  ...
  if tipo_Funcion == "Calcular tiempo de transferencia de archivo": 
    tamano_archivo = float(st.number_input("Ingresa el tamaño del archivo (MB)",value=0.10,min_value=0.1,step=0.1,format="%.2f",))
    velocidad = float(st.number_input("Ingresa la velocidad de transferencia (MBPS)",value=0.10,min_value=0.1,step=0.1,format="%.2f",))
